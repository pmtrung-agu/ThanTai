                ...
                <form action="{{ env('APP_URL') }}admin/user/update" method="post" id="dinhkemform" enctype="multipart/form-data">
                  ...
                  <div class="tab-content">
                                          ...
                                          <select class="select2 m-b-10 select2-multiple" name="roles[]" style="width: 100%" multiple="multiple" data-placeholder="Chọn quyền">
                                              ...
                                              @if($roles)
                                                @foreach($roles as $role)
                                                  @if(old('roles') !== null)
                                                    <option value="{{ $role }}" @if($user['roles'] !== null && in_array($role, old('roles'))) selected @endif >{{ $role }}</option>
                                                  @else
                                                    <option value="{{ $role }}" @if($user['roles'] !== null && in_array($role, $user['roles'])) selected @endif >{{ $role }}</option>
                                                  @endif
                                                @endforeach
                                              @endif
                                          </select>
                    ...
                    <div class="tab-pane" id="profile">
                      <div class="row">
                        <div class="col-12">
                          <textarea name="gioi_thieu" id="gioi_thieu" placeholder="Thông tin giớ thiệu" class="form-control summernote">{{ old('gioi_thieu') != null ? old('gioi_thieu') : $user['gioi_thieu'] }}</textarea>
                        </div>
                      </div>
                    </div>
                  </div>
