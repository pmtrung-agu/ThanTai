...
<div class="row">
  <div class="col-12">
    <div class="card-box table-responsive">
      <h3 class="m-t-0"><a href="{{ env('APP_URL') }}admin/seller/add" class="btn btn-primary"><i class="mdi mdi-folder-plus"></i></a> Danh sách các Doanh nghiệp</h3>
          ...
          @if($sellers)
            @foreach($sellers as $key => $user)
              <tr>
                  ...
                  <td class="text-center">
                      <a href="{{ env('APP_URL') }}admin/seller/delete/{{ $user['_id'] }}" class="btn btn-sm btn-danger" onclick="return confirm('Chắc chắn xóa?')"><i class="mdi mdi-delete"></i></a>
                      <a href="{{ env('APP_URL') }}admin/seller/edit/{{ $user['_id'] }}" class="btn btn-sm btn-info"><i class="mdi mdi-account-edit"></i></a>
                  </td>
              </tr>
            @endforeach
          @endif
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      $(".status").change(function(){
        ...
        $.get("{{ env('APP_URL') }}admin/seller/update-status/"+id);
      });
