...
<div class="row">
  <div class="col-12">
    <div class="card-box">
        <h3 class="m-t-0"><a href="{{ env('APP_URL') }}admin/article" class="btn btn-primary"><i class="mdi mdi-reply-all"></i></a> Chỉnh sửa Tin tức</h3>
        <form action="{{ env('APP_URL') }}admin/article/update" method="post" id="dinhkemform" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <textarea name="description" id="description" placeholder="Description" class="form-control">{{old('description') != null ? old('description') : $cat['description']}}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                <textarea name="contents" id="contents" placeholder="Nội dung chi tiết" class="form-control">{{old('contents') != null ? old('contents') : $cat['contents']}}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                              ...
                              <select name="id_category[]" id="id_category" multiple required class="select2" data-placeholder="Chọn Category">
                                @if($categories)
                                  @foreach($categories as $c)
                                    <option value="{{$c['_id']}}" @if(is_array($cat['id_category']) && in_array($c['_id'], $cat['id_category'])) selected @endif >{{$c['title']}}</option>}
                                    option
                                  @endforeach
                                @endif
                              </select>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group row">
                            ...
                            <div class="col-md-4">
                                <input type="number" id="order" name="order" class="form-control" placeholder="order" value="{{ old('order') != null ? old('order') : $cat['order'] }}" required />
                            </div>
...
@section('js')
  ...
  <script type="text/javascript">
       $(document).ready(function() {
          ...
          $('#contents').summernote({height: 300});
          $("#progressbar").hide();
